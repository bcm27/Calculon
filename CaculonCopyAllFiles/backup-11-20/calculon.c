...
static int compileProgram(Program *program) {
   ...
   for (itArg = program->arg; itArg != NULL; itArg = itArg->next) {
      ...
      printf("Exts: %s\n", extension);
      if(!extension) 
         printf("Error: %s, no file extension found\n", itArg->value);
      else {
         // if the file extension is a c file, and nothing else
         if (*(extension + 1) == 'c' && *(extension + 2) == '\0') {
            manyFiles++; /// increases the file count for mallocing
            printf("file found\n");         
            
            Arg *tempArg = malloc(sizeof(Arg));
            strncpy(tempArg->value, itArg->value, MAX_WORD_LENGTH);
            tempArg->next = NULL;

            cTemp = tempArg; // new argument
            cTemp->next = cHead;
            cHead = cTemp;
         }
      } // end else
   ...
   if (access("Makefile", F_OK) != -1) {
      ...
      *cmdTemp++ = "make";
      ...
      printf("- Attempting GCC compiling\n");
/*
      *cmdTemp++ = "gcc";
      for (cTemp = cHead; cTemp != NULL; cTemp = cTemp->next) {
          *cmdTemp++ = cTemp->value;
      }
   ...
   if ((childPID = fork()) < 0) // failure
      ...
      *cmdTemp = name;
      ...
      close(outFileDir);
		...
		// run whatever program we have cooked 
      execvp(*cmdLine, cmdLine);
...
static void makeTempDir(Program *program) {
...
static void transferFiles(Program *program) {
