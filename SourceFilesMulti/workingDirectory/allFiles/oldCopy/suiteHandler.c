...
int compileProgram(Program *program) {
   ...
   for (itArg = program->arg; itArg != NULL; itArg = itArg->next) {
      ...
      if (!extension) {
         ...
         if (*(extension + 1) == 'c' && *(extension + 2) == '\0') {  // if the file is a .c
            numbOfcurrentFiles++;
   ...
   cmdLine = cmdTemp = calloc(sizeof(char *), numbOfcurrentFiles + 4);
   ...
   //if(checkFileExists("Makefile") {
      ...
      *cmdTemp++ = "gcc";
      for (currentFile = HeadcurrentFile; currentFile != NULL; currentFile = currentFile->next) {
